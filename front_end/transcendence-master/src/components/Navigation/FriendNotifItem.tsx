...
export default function FriendNotifItem(props:any) {
    ...
    const accept = () => {
        props.socket.emit('acceptedFriendInvite', { roomN: notif.roomId, senderId: notif.senderId, type: notif.type })
    }

    const decline = () => {
        props.socket.emit('declinedFriendInvite', { senderId: notif.senderId })
    }
    ...
    if (senderData) {
        return (
            <div className="bg-slate-50 dark:bg-[#1A1C26] dark:text-white items-center justify-around w-full h-[50px] flex flex-row border-b-solid border-b-[1px] mt-[3px] border-b-white">
                ...
                <div className="w-[40%] text-center text-[15px]">{senderData.username} sent you a friend request</div>
                <div className="flex justify-around w-[40%] h-full items-center">
                    <button className="h-[55%] w-fit" onClick={accept}>
